...
def get_protein_embedding_and_mask(protein_seq, max_len=800):
    """
        vec: [max_len, dim]
        mask: [1, 1, max_len]
    """
    
    kmers = seq_to_kmers(protein_seq)
    n = len(kmers)
    m = max_len
    ...
    return vec.astype(np.float32), mask.astype(np.float32)
...
def main(txt_file):
    ...
    for i, point in enumerate(good_data):
        ...
        if x is not None and token is not None:
            ...
            protein_embed, p_mask = get_protein_embedding_and_mask(sequence)
            protein_list.append(protein_embed)
